...
for mall in mall_list:
    ...
    for index, row in train_tmp.iterrows():
        ...
        r['day_of_week'] = date(int(row['time_stamp'][0:4]),int(row['time_stamp'][5:7]),int(row['time_stamp'][8:10])).weekday() + 1
    ...
    #start training
    model = lgb.train(params, lgbtrain, 50, valid_sets=[lgbtrain], early_stopping_rounds=10)
    #predict
    df_test['label'] = model.predict(lgbtest, num_iteration=model.best_iteration)
    #label decoder
    ...
    result.to_csv('sublgb.csv',index=False)
